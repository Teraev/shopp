...
const btn_menu = document.querySelector('.tovar')
const menu = document.querySelector('.menu')
const cart_cont = document.querySelector('.items-cont')
const btn_close = document.querySelector('.close-dialog')

let cart = []


function commit(arr, place) {
    place.innerHTML = ""
    ...
    for (let item of arr) {
        ...
        let rate3 =document.createElement('div')
        ...
        img2.src = "./img/star.svg"
        span2.innerHTML = item.rating.rate
        rate2.append(img2, span2)
        
        rate3.classList.add('rate')
        img3.src = "./img/box.svg"
        span3.innerHTML = item.rating.count
        rate3.append(img3, span3)
        ...
        save_button.onclick = () => {
            ...
            if(selected) {
                ...
                cart.splice(idx , 1)
                save_button.innerHTML = "В избранное"
                save_button.classList.remove('blue')
            } else {
                cart.push(item)
                save_button.innerHTML = "Добавлено"
                save_button.classList.add('blue')
            }
...
action_btns.forEach((btn) => {
    btn.onclick = () => {
        let arr2 = ""
        if (btn.getAttribute('data-action') === "five") {
            arr2 = arr.slice(0, 5);
          } else {
            arr2 = arr;
          }
        commit(arr2, container)
    }
})

commit(arr, container)
function reload_menu(arr, place) {
    place.innerHTML = ""
    ...
    for (let item of arr) {
        ...
        inp.type = 'number'
        inp.value = '0'
        inp.classList.add('knopka')
        
        title.innerHTML = item.category
        price.innerHTML = item.price
        btn_plus.innerHTML = "+"
        btn_plus.classList.add('knopka')
        btn_minus.innerHTML = "-"
        btn_minus.classList.add('knopka')
        remove.innerHTML = "remove"
        remove.classList.add("udalit")
        ...
        cart_item.append(left_side, right_side)
        left_side.append(img, Names)
        Names.append(title, price)
        right_side.append(counter, remove)
